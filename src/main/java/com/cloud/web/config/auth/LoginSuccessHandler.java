 ...
 *
 *  spring security에서 제공하는 redirect url를 사용하지 않는 경우, 권한이 없어서 로그인 페이지로 넘어가기 직전에 요청한 페이지의 url를 담을려고 한다.
 *  RequestCache 인터페이스를 이용해, 사용자 요청 정보들이 들어 있는 SavedRequest 클래스 객체를 세션에 저장하게 된다.
 *  그럼 우리는 RequestCache 객체를 생성해 SavedRequest 객체를 가져와서 로그인 화면을 보기 전에 방문했던(요청했던) URL 정보를 가져오면 된다.
 *
