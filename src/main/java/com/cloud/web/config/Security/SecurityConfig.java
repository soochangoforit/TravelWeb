...
public class SecurityConfig extends WebSecurityConfigurerAdapter {
    ...
    protected void configure(HttpSecurity http) throws Exception {
        ...
        http.csrf().disable()
                .authorizeRequests()
                .antMatchers("/foodForm/**","/myPage/**").authenticated() // 여기서 굉장히 중요한 기능이!! 로그인 하지 않은 user 사용자가 해당 주소로 갈려고 하면 최소한은 로그인은 되어야 하기 때문에 /login으로 넘어가고, 넘어가서 로그인이 인증이 되면 리다이렉트 주소로 /user로 온다. (접속을 시도한 시점의 URL로 온다.)
                .antMatchers("/adminPage").access("hasRole('ROLE_ADMIN')") // 권한이 없으면 403 나와야 한다. hasRole은 앞에 "ROLE_"을 붙인 상태에서 확인하고, hasAnyAuthority는 붙지 않는다.?? 그래서 접근은 가능하지만 @Secured 기능에서 사용되지 않는다.
